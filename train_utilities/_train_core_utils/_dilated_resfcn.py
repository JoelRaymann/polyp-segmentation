"""
Scripts to train a single loop of Dilated ResFCN
"""
...
def _run_dilated_resfcn(model, x, y_gnd, training=True):
    # Gradient Tape it
    with tf.GradientTape() as tape:
        ...
        # Find the loss
        loss_value = tf.reduce_mean(tf.keras.losses.categorical_crossentropy(y_gnd, y_pred))
        loss_value += sum(model.losses)

    # Get the polyp class alone
    # y_gnd_label = tf.expand_dims(tf.argmax(y_gnd, axis=-1), axis=-1)
    # y_pred_label = tf.expand_dims(tf.argmax(y_pred, axis=-1), axis=-1)
    y_gnd_label = tf.expand_dims(y_gnd.numpy()[:, :, :, -1], axis=-1)
    y_pred_label = tf.expand_dims(y_pred.numpy()[:, :, :, -1], axis=-1)

    # Run the metric
    f1_score = tf.reduce_mean(tf.keras.metrics.categorical_accuracy(y_gnd, y_pred))
    iou_coe = metrics.iou_coe(y_gnd_label, y_pred_label)
    dice_coe = metrics.dice_coe(y_gnd_label, y_pred_label)
...
def train_dilated_resfcn(model: tf.keras.Model, dataset: tf.data.Dataset, epoch_steps: int, metrics_tracker: tuple,
                        optimizer=None, **kwargs):
    """
    Function to train or test in a single loop in the UNet model for dice loss. NOTE: If optimizer is set as None,
    then the loop acts as a testing loop.

    Parameters
    ----------
    model : tf.keras.Model
        A built U-Net model for training.
        ...
        with tqdm(total=epoch_steps) as pbar:
            for X, y in dataset:
                ...
                if optimizer is not None:
                    ...
                    loss_values, grad, metrics_eval = _run_dilated_resfcn(model, X, y, training=True)
                    optimizer.apply_gradients(zip(grad, model.trainable_variables))

                else:
                    # Get the loss and predictions
                    loss_values, grad, metrics_eval = _run_dilated_resfcn(model, X, y, training=False)
