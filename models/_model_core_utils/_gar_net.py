...
def GARNet(input_size: tuple, test_mode=False):
    ...
    x, skip2 = _modified_residual_block(x, [128, 128, 128], "L2", skip_conn_type="conv", stride=2, activation="elu",
                                        return_skip=True)

    # Level - 3 Xception down
    x, skip3 = _modified_residual_block(x, [256, 256, 256], "L3", skip_conn_type="conv", stride=2, activation="elu",
                                        return_skip=True)

    # Level - 4 Xception down
    x, skip4 = _modified_residual_block(x, [512, 512, 512], "L4", skip_conn_type="conv", stride=2, activation="elu",
                                        return_skip=True)

    # Bottleneck
    x = _modified_residual_block(x, [1024, 1024, 1024], "Bottleneck_entry", skip_conn_type="none", stride=1,
                                 activation="elu")
    x = _modified_residual_block(x, [1024, 1024, 1024], "Bottleneck_Res0", skip_conn_type="sum", stride=1,
                                 activation="elu")
    x = _modified_residual_block(x, [1024, 1024, 1024], "Bottleneck_Res1", skip_conn_type="sum", stride=1,
                                 activation="elu")
    x = _modified_residual_block(x, [1024, 1024, 1024], "Bottleneck_Res2", skip_conn_type="sum", stride=1,
                                 activation="elu")
    ...
    x = _modified_residual_block(x, [512, 512, 512], "U4", skip_conn_type="conv", stride=1, activation="elu")
    ...
    x = _modified_residual_block(x, [256, 256, 256], "U3", skip_conn_type="conv", stride=1, activation="elu")
    ...
    x = _modified_residual_block(x, [128, 128, 128], "U2", skip_conn_type="conv", stride=1, activation="elu")
