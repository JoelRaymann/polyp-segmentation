"""
Scripts to train a single loop of UNet
"""
...
def _run_unet_dice(model, x, y_gnd, training=True):
    # Gradient Tape it
...
def train_unet_dice(model: tf.keras.Model, dataset: tf.data.Dataset, epoch_steps: int, metrics_tracker: tuple,
        ...
        with tqdm(total=epoch_steps) as pbar:
            for X, y in dataset:
                ...
                if optimizer is not None:
                    ...
                    loss_values, grad, metrics_eval = _run_unet_dice(model, X, y, training=True)
                    optimizer.apply_gradients(zip(grad, model.trainable_variables))

                else:
                    # Get the loss and predictions
                    loss_values, grad, metrics_eval = _run_unet_dice(model, X, y, training=False)
