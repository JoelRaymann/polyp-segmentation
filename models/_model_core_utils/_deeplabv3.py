...
if tf.__version__[0] == "2":
    ...
    from tensorflow.keras.layers import Layer, InputSpec, Conv2D, DepthwiseConv2D, UpSampling2D, ZeroPadding2D, Lambda, \
        AveragePooling2D, Input, Activation, Concatenate, Add, Reshape, BatchNormalization, Dropout
...
def Deeplabv3(weights='pascal_voc', input_tensor=None, infer=False,
    ...
    b4 = Lambda(lambda x: tf.image.resize(x, size=(
    int(np.ceil(input_shape[0] / OS)), int(np.ceil(input_shape[1] / OS)))))(b4)
    ...
    if backbone == 'xception':
        ...
        x = Lambda(lambda x: tf.image.resize(x, size=(
