"""
Scripts to train a single loop of UNet Attention
"""
...
def _run_unet_attn(model, x, y_gnd, training=True):
    # Gradient Tape it
    with tf.GradientTape() as tape:
        ...
        # Get the predictions
        y_pred, attn1_map, attn2_map, attn3_map, attn4_map = Y_pred
...
def train_unet_attn(model: tf.keras.Model, dataset: tf.data.Dataset, epoch_steps: int, metrics_tracker: tuple,
                    optimizer=None, **kwargs):
    """
    Function to train or test in a single loop in the UNet Attention model for dice loss. NOTE: If optimizer is set as None,
        ...
        with tqdm(total=epoch_steps) as pbar:
            for X, y in dataset:
                ...
                if optimizer is not None:
                    ...
                    loss_values, grad, metrics_eval = _run_unet_attn(model, X, y, training=True)
                    optimizer.apply_gradients(zip(grad, model.trainable_variables))

                else:
                    # Get the loss and predictions
                    loss_values, grad, metrics_eval = _run_unet_attn(model, X, y, training=False)
