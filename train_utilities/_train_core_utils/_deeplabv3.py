"""
Scripts to train a single loop of DeepLabv3
"""
...
def _run_deeplabv3(model, x, y_gnd, training=True):
    # Gradient Tape it
...
def train_deeplabv3(model: tf.keras.Model, dataset: tf.data.Dataset, epoch_steps: int, metrics_tracker: tuple,
                    optimizer=None, **kwargs):
    """
    Function to train or test in a single loop in the DeepLabv3 model for dice loss. NOTE: If optimizer is set as None,
    then the loop acts as a testing loop.

    Parameters
    ----------
    model : tf.keras.Model
        A built DeepLabv3 model for training.
        ...
        with tqdm(total=epoch_steps) as pbar:
            for X, y in dataset:
                ...
                if optimizer is not None:
                    ...
                    loss_values, grad, metrics_eval = _run_deeplabv3(model, X, y, training=True)
                    optimizer.apply_gradients(zip(grad, model.trainable_variables))

                else:
                    # Get the loss and predictions
                    loss_values, grad, metrics_eval = _run_deeplabv3(model, X, y, training=False)
