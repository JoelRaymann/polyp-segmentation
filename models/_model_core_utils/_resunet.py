"""
Script to implement the ResU-Net Architecture for training/testing
"""
...
def _residual_block(X, filters: int, kernel_size: int, activation: str, name: str, batch_norm=True):

    conv1 = tf.keras.layers.Conv2D(filters, kernel_size, activation=None, padding="same", name="res_conv1_" + name)(X)
    if batch_norm:
        conv1 = tf.keras.layers.BatchNormalization(axis=-1)(conv1)
    conv1 = tf.keras.layers.Activation(activation)(conv1)
    ...
    conv1 = _conv_block(conv1, filters, kernel_size, activation, name="res_conv_main" + name)
    conv1 = tf.keras.layers.Add()([skip, conv1])

    return conv1


def ResUNet(input_size: tuple, test_mode=False):
    """
    Function to build the ResU-Net architecture adapted for Dice index training.
    ...
    # Level - 2 Down
    L2_conv = _residual_block(L1_pool, 128, 3, "elu", name="L2", batch_norm=True)
    L2_pool = tf.keras.layers.MaxPool2D(pool_size=(2, 2), name="pool_L2")(L2_conv)

    # Level - 3 Down
    L3_conv = _residual_block(L2_pool, 256, 3, "elu", name="L3", batch_norm=True)
    L3_pool = tf.keras.layers.MaxPool2D(pool_size=(2, 2), name="pool_L3")(L3_conv)

    # Level - 4 Down
    L4_conv = _residual_block(L3_pool, 512, 3, "elu", name="L4", batch_norm=True)
    L4_pool = tf.keras.layers.MaxPool2D(pool_size=(2, 2), name="pool_L4")(L4_conv)

    # Bridge
    B_conv = _residual_block(L4_pool, 1024, 3, "elu", name="Bridge", batch_norm=True)
