"""
Script to prepare the data for the GAR-Net model
"""
...
# Handling GAR-Net
def _load_data_gar_net(img, seg):
    """
    ...
    attn1 = seg.copy()
    attn2 = cv.resize(attn1, (image_width // 2, image_height // 2), interpolation=cv.INTER_CUBIC)
    attn3 = cv.resize(attn1, (image_width // 4, image_height // 4), interpolation=cv.INTER_CUBIC)
    attn4 = cv.resize(attn1, (image_width // 8, image_height // 8), interpolation=cv.INTER_CUBIC)
